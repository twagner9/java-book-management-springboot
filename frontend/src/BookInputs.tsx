...
export function BookInputs(props: { onBookAdded: (book: Book) => void }) {
  ...
  const isAuthorValid =
    newBook.authorLast.length > 1 &&
    newBook.authorLast.length <= 20 &&
    newBook.authorFirst.length > 1 &&
    newBook.authorFirst.length <= 20;
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (!isNaN(newValue) && newValue >= 1 && newValue <= 99) {
                setBook((b) => ({
                  ...b,
                  numCopies: (e.target as HTMLInputElement).valueAsNumber,
                }));
